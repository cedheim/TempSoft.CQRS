...
namespace TempSoft.CQRS.Tests.ServiceFabric.Events.EventStreamService
{
    public abstract class EventStreamServiceTestBase
    {
        ...
        protected readonly EventStreamDefinition EventStreamDefinition = new EventStreamDefinition(EventStreamName, new EventFilter
        {
        ...
        {
            var context = MockStatefulServiceContextFactory.Create(MockCodePackageActivationContext.Default, ServiceType, ServiceUri,PartitionId, 0 );
            var partitionInformation = MockQueryPartitionFactory.CreateStatefulPartition(MockQueryPartitionFactory.CreateNamedPartitonInfo(EventStreamName, context.PartitionId), 1, 1, HealthState.Ok, ServicePartitionStatus.Ready, TimeSpan.MaxValue, new Epoch());
            ...
            Service = new CQRS.ServiceFabric.Events.EventStreamService(context, StateManager, StreamStateManager, FabricClient, EventStore, EventStreamRegistry);
