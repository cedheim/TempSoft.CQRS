...
namespace TempSoft.CQRS.Tests.CosmosDb.Events.EventStore
{
    ...
    public class When_listing_all_events_after_a_certain_time
    {
        ...
        public async Task OneTimeSetUp()
        {
            ...
            _events = new IEvent[]
            {
                new ChangedAValue(Data.AValue) { Version = 0, AggregateRootId = Data.AggregateRootId1, EventGroup = nameof(AThingAggregateRoot), Timestamp = Data.Timestamp},
                new ChangedAValue(Data.AValue) { Version = 0, AggregateRootId = Data.AggregateRootId2, EventGroup = nameof(AThingAggregateRoot), Timestamp = Data.Timestamp.AddHours(1) },
                new ChangedAValue(Data.AValue) { Version = 1, AggregateRootId = Data.AggregateRootId1, EventGroup = nameof(AThingAggregateRoot), Timestamp = Data.Timestamp.AddHours(2) },
                new ChangedBValue(Data.BValue) { Version = 1, AggregateRootId = Data.AggregateRootId2, EventGroup = nameof(AThingAggregateRoot), Timestamp = Data.Timestamp.AddHours(3) },
                new ChangedBValue(Data.BValue) { Version = 2, AggregateRootId = Data.AggregateRootId1, EventGroup = nameof(AThingAggregateRoot), Timestamp = Data.Timestamp.AddHours(4) },
                new ChangedBValue(Data.BValue) { Version = 2, AggregateRootId = Data.AggregateRootId2, EventGroup = nameof(AThingAggregateRoot), Timestamp = Data.Timestamp.AddHours(5) },
            };
            ...
            A.CallTo(() => _client.CreateDocumentQuery<EventPayloadWrapper>(A<Uri>.Ignored, A<FeedOptions>.Ignored))
                .Returns(_events.Select(e => new EventPayloadWrapper(e){ Timestamp = e.Timestamp.ToUnixTime() }).AsQueryable().OrderBy(e => e.Version));
