...
namespace TempSoft.CQRS.Tests.Mocks
{
    public class InMemoryActorStateManager : IActorStateManager
    {
        ...
        {
            return Task.Run(
                () => (T) _cache.AddOrUpdate(stateName, addValue.Clone(),
                    (s, o) => updateValueFactory(s, (T) o).Clone()), cancellationToken);
        }

        public Task AddStateAsync<T>(string stateName, T value,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache.TryAdd(stateName, value.Clone()));
        }

        public Task ClearCacheAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache = new ConcurrentDictionary<string, object>(_saved));
        }

        public Task<bool> ContainsStateAsync(string stateName,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache.ContainsKey(stateName), cancellationToken);
        }

        public Task<T> GetOrAddStateAsync<T>(string stateName, T value,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => (T) _cache.GetOrAdd(stateName, value.Clone()), cancellationToken);
        }

        public Task<T> GetStateAsync<T>(string stateName,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => (T) _cache[stateName].Clone(), cancellationToken);
        }

        public Task<IEnumerable<string>> GetStateNamesAsync(
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache.Keys.ToArray().AsEnumerable());
        }

        public Task RemoveStateAsync(string stateName, CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache.TryRemove(stateName, out _), cancellationToken);
        }

        public Task SaveStateAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _saved = new ConcurrentDictionary<string, object>(_cache));
        }

        public Task SetStateAsync<T>(string stateName, T value,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache[stateName] = value.Clone());
        }

        public Task<bool> TryAddStateAsync<T>(string stateName, T value,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return Task.Run(() => _cache.TryAdd(stateName, value.Clone()), cancellationToken);
        }
        ...
        {
            return Task.Run(
                () => new ConditionalValue<T>(_cache.TryGetValue(stateName, out var value), (T) value.Clone()),
                cancellationToken);
        }
