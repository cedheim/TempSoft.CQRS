...
namespace TempSoft.CQRS.CosmosDb.Tests.Commands.CommandRegistry
{
    ...
    public class IntegrationTests
    {
        ...
        public async Task OneTimeSetUp()
        {
            ...
            await _repository.Save(Data.AggregateRootId4, Data.CommandIds4);
        }
        ...
        [Test]
        public async Task When_getting_command_ids_with_a_strange_aggregate_root_id()
        {
            var commandIds = (await _repository.Get(Data.AggregateRootId4, CancellationToken.None)).ToArray();
            commandIds.Should().HaveCount(Data.CommandIds4.Length);
            commandIds.Should().OnlyContain(id => Data.CommandIds4.Contains(id));
        }

        [Test]
        public async Task When_getting_a_command_by_identifier()
        {
            var identifier = CommandRegistryWrapper.CreateIdentifier(Data.CommandIds1[0]);
            var uri = UriFactory.CreateDocumentUri(Data.DatabaseId, Data.Collectionid, identifier);

            var document = (await _client.ReadDocumentAsync(uri, new RequestOptions
            {
                PartitionKey = new PartitionKey(Data.AggregateRootId1)
            })).Resource;

            document.GetPropertyValue<Guid>("CommandId").Should().Be(Data.CommandIds1[0]);
            document.GetPropertyValue<string>("PartitionId").Should().Be(Data.AggregateRootId1);
            document.GetPropertyValue<string>("DocumentType").Should().Be(CommandRegistryWrapper.DocumentTypeName);
        }
    }
}