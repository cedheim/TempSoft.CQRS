...
namespace TempSoft.CQRS.ServiceFabric.Events
{
    public class EventStreamService : StatefulService, IEventStreamService
    {
        ...
        public EventStreamService(StatefulServiceContext serviceContext,
            IReliableStateManagerReplica reliableStateManagerReplica, IEventStreamStateManager streamStateManager,
            IFabricClient fabricClient, IEventStore eventStore, IEventStreamRegistry eventStreamRegistry) : base(
        ...
        public EventStreamService(StatefulServiceContext serviceContext, IEventStreamStateManager streamStateManager,
            IFabricClient fabricClient, IEventStore eventStore, IEventStreamRegistry eventStreamRegistry) : base(
            serviceContext)
        {
            _streamStateManager = streamStateManager;
        ...
        public async Task<EventMessage> Read(TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            while (end > DateTime.UtcNow)
            {
                try
                {
                    ...
                    {
                        var conditionalResult = await queue.TryDequeueAsync(transaction, TimeSpan.FromMilliseconds(100),
                            cancellationToken);
                        await transaction.CommitAsync();

                        if (conditionalResult.HasValue) return conditionalResult.Value;
                    }
                }
        ...
        protected override async Task OnOpenAsync(ReplicaOpenMode openMode, CancellationToken cancellationToken)
        {
            ...
            var namedPartition = (NamedPartitionInformation) partitions
                .FirstOrDefault(p => p.PartitionInformation.Id == Context.PartitionId)?.PartitionInformation;
            if (namedPartition == null)
                throw new EventStreamInitializationException(
                    $"Unable to find name for partition {Context.PartitionId}");
        ...
        protected override async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            if (status != EventStreamStatus.Initialized) await Initialize(cancellationToken);
        ...
        private async Task Initialize(CancellationToken cancellationToken)
        {
            ...
            var eventFilter = new EventStoreFilter
            {
                EventGroups = _definition.Filter.EventGroups?.ToArray(),
                EventTypes = _definition.Filter.EventTypes?.Select(t => t.ToFriendlyName())?.ToArray()
            };

            await _eventStore.List(eventFilter,
                async (@event, token) => { await AddEventToStream(new EventMessage(@event), token); },
                cancellationToken);
